...
```mermaid
flowchart LR
  T[long-horizon task] --> B[BPE harness state]
  B --> A[policy chooses read/update/consolidate]
  A --> E[external workspace executes]
  E --> R[task reward + access cost]
  R --> G[cost-aware GRPO]
  G --> A
```

原文 BPE state、SFT、cost-aware GRPO 和 annealing/evolution 曲线见 [arXiv HTML](https://arxiv.org/html/2608.05446)。
