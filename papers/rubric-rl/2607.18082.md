...
- **训练/评测模型**：训练 judge 使用 Qwen3-32B；最终评测 judge 使用 GPT-4o-mini；policy backbone 待核对。
- **Benchmark / 数据**：RaR-Medicine、RaR-Science；同域测试以及跨域 HealthBench、LLMEval-Med、ResearchQA。
- **对比基线**：rubric-based RL 与其他 criteria exploration/credit 方法。
- **指标**：任务最终得分、UC/SC 覆盖与 token-level credit 分析、达到目标所需 optimization steps。
- **主要结果**：CriPO 在医学/科学任务上优于 rubric RL baseline，并约少用一半优化步数达到更强最终表现。
