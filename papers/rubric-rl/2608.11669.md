...
```mermaid
flowchart LR
  G[rollout group] --> M[随机 mask criteria]
  M --> J[训练 judge 计算部分 rubric reward]
  J --> A[组内 GRPO advantage]
  A --> U[更新 policy]
  U --> E[完整 rubric 评估]
```

原文主图、dropout 变体和实验曲线见 [arXiv HTML](https://arxiv.org/html/2608.11669)。
