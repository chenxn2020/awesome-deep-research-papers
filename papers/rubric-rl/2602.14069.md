...
- **训练/评测模型**：refinement policy 使用 Qwen3-30B-A3B；oracle judge 使用 Qwen3-235B-A22B-Instruct-2507。
- **Benchmark / 数据**：RM-Bench、JudgeBench、RewardBench v2、PPE Preference；另有 pairwise RL 下游评测。
- **对比基线**：scalar reward model、trained generative judge，以及不做 rubric refinement 的设置。
- **指标**：reward-model/judge 的偏好准确性与下游 RL 表现；精确分数待核对。
- **主要结果**：OpenRS 在四个 reward-modeling benchmark 上超过上述基线，接入 pairwise RL 后下游表现稳定提升。
