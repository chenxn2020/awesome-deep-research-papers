...
- **训练/评测模型**：评测 25 个 state-of-the-art 多模态/Deep Research 模型，具体型号以论文表格为准（待核对）。
- **Benchmark / 数据**：140 个 image-text bundle 任务，覆盖 21 个领域。
- **对比基线**：不同模型/系统的端到端 Deep Research 结果。
- **指标**：FLAE（报告质量）、TRACE（引用与检索证据对齐）、MOSAIC（文本-视觉一致性）。
- **主要结果**：模型在生成质量、citation discipline 与 multimodal grounding 之间存在系统性 trade-off；精确排名待核对。
