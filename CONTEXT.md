...
**Manual Submission**:
由用户主动筛选并提交、要求查阅原文后直接整理入库的论文、研究型官方博客或有价值资源；保留发现该内容的推送出处。原文无法唯一核实时不入库，标记“待补原文”。
_Avoid_: 自动发现候选、仅保存链接的待读项、没有原文依据的二手解读

**Curated Paper**:
完成全文审核、符合分类标准并正式进入仓库的论文；自动发现和用户主动提交都适用。
_Avoid_: 候选论文、只有二手摘要的条目
...
**Others**:
超出四个论文 Track 和 Official Lab Blogs、但由用户主动提交且值得整理的资源模块；可按主题建立子目录，收录非论文文章、项目、工具或讨论。
